...
for (const locale of locales) {
  ...
  let overrides = {};
  try {
    overrides = JSON.parse(await readFile(join(overridesRoot, `${locale}.json`), "utf8"));
  } catch (error) {
  ...
  const invalidOverrides = Object.entries(overrides).filter(([key, value]) => !(key in english) || catalog[key] !== value);
  if (missing.length || stale.length || invalidPlaceholders.length || leakedSentinels.length || invalidOverrides.length) {
    failed = true;
    console.error(`${locale}: ${missing.length} missing, ${stale.length} stale, ${invalidPlaceholders.length} placeholder mismatches, ${leakedSentinels.length} leaked sentinels, ${invalidOverrides.length} invalid overrides`);
  } else {
